...
#CVM_DEBUG_STACKTRACES	?= true # Clamp
CVM_DEBUG_STACKTRACES	?= $(CVM_DEBUG) 
...
# command line flags
#
# LG DTV - BY Clamp (PROFILE CALL COUNT) *****
ifeq ($(PROFILE_CALL_COUNT), true)
    CVM_DEFINES   += -DAOTC_PROFILE_CALL_COUNT
    AOTC_BUILDTIME_CLASSES = -none
    USE_PROFILE   = false
    CVM_JIT       = false
#    CVM_AOT       = false
endif
ifeq ($(USE_PROFILE), true)
    CVM_DEFINES   += -DAOTC_USE_PROFILE
endif
# ********************************************
# AOTC - BY Clamp
ifneq ($(AOTC_BUILDTIME_CLASSES), -none)
    CVM_DEFINES   += -DSERVER_AOTC
ifeq ($(AOTC_METHOD_ARGUEMENT_PASSING), -cni)
    CVM_DEFINES   += -DSERVER_AOTC_CNI
endif
ifeq ($(AOTC_METHOD_ARGUEMENT_PASSING), -jni)
    CVM_DEFINES   += -DSERVER_AOTC_JNI
endif
ifeq ($(AOTC_METHOD_ARGUEMENT_PASSING), -mix)
    CVM_DEFINES   += -DSERVER_AOTC_MIX
endif
ifeq ($(AOTC_METHOD_ARGUEMENT_PASSING), -wrapper)
    CVM_DEFINES   += -DSERVER_AOTC_WRAPPER
endif

ifeq ($(AOTC_EXCEPTION_HANDLING), -setjmp_prolog)
    CVM_DEFINES   += -DSERVER_AOTC_SETJMP_EXCEPTION_HANDLING
endif
ifeq ($(AOTC_EXCEPTION_HANDLING), -setjmp_try)
    CVM_DEFINES   += -DSERVER_AOTC_SETJMP_EXCEPTION_HANDLING
endif
endif
...
CCCFLAGS 	= -fno-rtti
ifeq ($(CVM_OPTIMIZED), true)
#CCFLAGS_SPEED	= $(CCFLAGS) -O0
#CCFLAGS_SPACE	= $(CCFLAGS) -O0
#CCFLAGS_SPEED	= $(CCFLAGS) -O2
#CCFLAGS_SPACE	= $(CCFLAGS) -O1
