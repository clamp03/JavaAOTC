...
;;;     It runs in 45 seconds [to run a single iteration]
;;;     on herzberg under Jess; it works correctly, though
;;;
;;; Modified 10/23/97 by Walter Bays, Sun Microsystems
;;; To make it run longer for a benchmark, make it "iterate"
;;; However it's not solving exactly the same problem each time
;;; For each iteration I add a new set of facts, but I do not
;;; retract the facts from the previous iteration. The only difference
;;; between the new facts and the prior facts is the iteration number.
;;; No matter to the inference engine which has to sort through all
;;; of the facts from all iterations to find a set which match the
;;; predicates for its' rules. So increasing numbers of iterations
;;; should get increasingly difficult. At some point there could
;;; be a problem that the fact set becomes uncharacteristically large,
;;; but that seems not to be the case for any number of iterations
;;; we'd want to run for a benchmark, comparing it to the rule sets of 
;;; big CLIPS applicatons. I suppose it could also be a problem that
;;; the rule set is too "regular", but I'm not sure that makes much
;;; of a difference at the JVM level anyway.
;;;======================================================
...
(defrule generate-combinations
  ...
  (letter ?a)
  =>
  (assert (combination ?n ?a ?x)))

(defrule find-solution
  ?think <- (state think)
  (iteration ?it)
  (combination ?it D ?d )
  (combination ?it T ?t&~?d&:(= (mod (+ ?d ?d) 10) ?t))
  (combination ?it L ?l&~?d&~?t)
  (combination ?it R ?r&~?d&~?t&~?l&:(= (mod (+ ?d ?d
                                            (* 10 ?l) (* 10 ?l))
                                         100)
                                    (+ (* 10 ?r) ?t)))
  (combination ?it A ?a&~?d&~?t&~?l&~?r)
  (combination ?it E ?e&~?d&~?t&~?l&~?r&~?a&:(= (mod (+ ?d ?d
                                                    (* 10 ?l) (* 10 ?l)
                                                    (* 100 ?a) (* 100 ?a))
                                                 1000)
                                            (+ (* 100 ?e) (* 10 ?r) ?t)))
  (combination ?it N ?n&~?d&~?t&~?l&~?r&~?a&~?e)
  (combination ?it B ?b&~?d&~?t&~?l&~?r&~?a&~?e&~?n&:(= (mod (+ ?d ?d
                                                            (* 10 ?l) (* 10 ?l)
                                                            (* 100 ?a) (* 100 ?a)
                                                            (* 1000 ?r) (* 1000 ?n))
                                                         10000)
                                                    (+ (* 1000 ?b) (* 100 ?e) (* 10 ?r) ?t)))
  (combination ?it O ?o&~?d&~?t&~?l&~?r&~?a&~?e&~?n&~?b)
  (combination ?it G ?g&~?d&~?t&~?l&~?r&~?a&~?e&~?n&~?b&~?o&:(= (+ ?d ?d
                                                            (* 10 ?l) (* 10 ?l)
                                                            (* 100 ?a) (* 100 ?a)
                                                            (* 1000 ?r) (* 1000 ?n)
                                                            (* 10000 ?e) (* 10000 ?o)
  ...
  (printout t "check #" ?it "= " (+ ?g (+ ?r (+ ?d (+ ?t ?it)))) crlf)
  (retract ?think)
  (assert (state count))
  )  
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defrule find-solution-z
  ...
  (avh (a color) (v green) (h ?c3&~?c2&~?c1&:(eq (+ ?c2 1) ?c3)))
...
(defrule generate-combinations-z
   ?f <- (value ?s ?e)
   =>
   (retract ?f)
