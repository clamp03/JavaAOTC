...
class Pattern {
  ...
  public int size() { return nvalues + RU.FIRST_SLOT; }
  ...
  public Pattern(String name, int ordered, Rete engine) throws ReteException {
    ...
    if (_ordered == RU.ORDERED_FACT)
      ...
      nvalues = (deft.size() - RU.FIRST_SLOT) / RU.DT_SLOT_SIZE;
      tests = new Test1[nvalues][];
      slotlengths = new int[nvalues];
    }
  ...
  public void SetMultislotLength(String slotname, int length) 
    ...
    for (i=RU.FIRST_SLOT; i< deft.size(); i+=RU.DT_SLOT_SIZE)
      if (deft.get(i).StringValue().equals(slotname))
        break;
    if (i >= deft.size())
      throw new ReteException("Pattern::SetMultislotLength",
  ...
  public void AddTest(int slotname, Value value,  int subidx, boolean negated)
    ...
    if (negated)
      tests[idx][j] = new Test1(Test1.NEQ, i, subidx, value);
    else
      tests[idx][j] = new Test1(Test1.EQ, i, subidx, value);
