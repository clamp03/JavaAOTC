...
public class CVMWriter implements CoreImageWriter, Const, CVMConst {
    ...
    protected String        jniFileName = null;
    protected CCodeWriter   jniOut;
    protected OutputStream  jjj;
    boolean jniUpdate = false;
    // ************************************
    // LG DTV - BY Clamp (CVM AOT) *****
    protected String        shaFileName = null;
    protected CCodeWriter   shaOut;
    protected OutputStream  sss;
    boolean cvmAot = false;
    // **********************************
    ...
    // LG DTV - BY Clamp (JNI Update & CVM AOT) *****
    public void init(boolean classDebug, boolean lossless,
                     ClassnameFilterList nativeTypes, boolean verbose,
                     int maxSegmentSize, boolean mutableMBs, boolean jniUpdate, boolean cvmAot) {
    // ************************************
    // LG DTV - BY Clamp (JNI Update, Commented) *****
    // public void init(boolean classDebug, boolean lossless,
    //                 ClassnameFilterList nativeTypes, boolean verbose,
	...
	if ( ! segmentedOutput ){
        ...
        if(jniUpdate || AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.WRAPPER_TYPE) {
            jniFileName = filename + "JNI.c";
            jjj = new java.io.FileOutputStream( jniFileName );
            jniOut = new CCodeWriter(jjj);
            listOut.println( jniFileName );
        }
        // ************************************
        // LG DTV - BY Clamp (CVM AOT) *****
        if(cvmAot) {
            shaFileName = filename + "Sha1.c";
            sss = new java.io.FileOutputStream( shaFileName);
            shaOut = new CCodeWriter(sss);
            listOut.println( shaFileName );
        }
        // **********************************
	...
	for ( int i = 0; i < nmethod; i++ ){
        ...
        String invoker;
        if(isAbstract) {
            invoker = "CVM_INVOKE_ABSTRACT_METHOD";
        } else {
            if(isNative) {
                if(isClassCNI(c.classInfo.className)) {
                    if((meth.method.access & Const.ACC_AOTC)!=0) {
                        invoker = "CVM_INVOKE_AOTC_METHOD";
                        //invoker = synchro ? "CVM_INVOKE_JNI_SYNC_METHOD" : "CVM_INVOKE_JNI_METHOD";
                    } else {
                        invoker = "CVM_INVOKE_CNI_METHOD";
                    }
                } else {   
                    if((meth.method.access & Const.ACC_AOTC)!=0) {
                        invoker = "CVM_INVOKE_AOTC_METHOD";
                ...
                invoker = synchro ? "CVM_INVOKE_JAVA_SYNC_METHOD" : "CVM_INVOKE_JAVA_METHOD";
            }
        }
        // ****************************************************************************
        /* AOTC - BY Clamp (commented) ************************************************
	...
	for ( int i = 0; i < nmethod; i++ ){
	    ...
	    if ( mi.code != null ){
            // AOTC - BY Clamp ****************************************************************************************
            String className = c.classInfo.className;
            if((mi.access & Const.ACC_AOTC)!=0) {
                // Call Arg - BY Clamp ***********
                if(AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.CNI_TYPE) {
                    if(isClassCNI(className)) {
                        AOTCWriter.translate(c.classInfo, mi, "Java_" + mi.getNativeName(true).substring(5), nativeTypes);//ikhyun 061221
                    } else {
                        AOTCWriter.translate(c.classInfo, mi, mi.getNativeName(true), nativeTypes);
                    }
                }else {
                    if(isClassCNI(className)) {
                        //System.err.println("***CNI**** Class[" + className + "] Method [" + mi.getNativeName(true) + "]");
                        AOTCWriter.translate(c.classInfo, mi, "CNI" + mi.getNativeName(true).substring(5), nativeTypes);
                    } else {
                        //System.err.println("********** Class[" + className + "] Method [" + mi.getNativeName(true) + "]");
                        AOTCWriter.translate(c.classInfo, mi, mi.getNativeName(true), nativeTypes);
                    }
                }
                // ********************************
	...
	for ( int i = 0; i < nmethod; i++ ){
        ...
        if(ClassFilter.AOTC_LIST != 0 && AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.WRAPPER_TYPE) {
            //classOut.print(",NULL),\n");
            classOut.print(",NULL, NULL),\n");
        }else {
            classOut.print("),\n");
        }
	    } else if ( isNative ){
		if (isClassCNI(c.classInfo.className)) {
            ...
            String eniName;
            if(AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.CNI_TYPE) {
                eniName = jniName.substring(5);//AOTC
                if((meth.method.access & Const.ACC_AOTC) != 0) {
                    eniName = "Java_" + eniName; //061221
                }   
                else{
                    eniName = "CNI"+eniName; //ikhyun
                } 
            }else {
                eniName = "CNI" + jniName.substring(5);
            }
            /*
            ...
            */
            ...
            if(ClassFilter.AOTC_LIST != 0 && AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.WRAPPER_TYPE) {
                if((meth.method.access & Const.ACC_AOTC) != 0) {
                    //classOut.print(", (void*)&ParseArgAndCall_"+eniName+"),\n");
                    if((meth.method.access & Const.ACC_STATIC) != 0) {
                        classOut.print(", (void*)&ParseArgAndCall_"+eniName+", NULL),\n");
                    }else {
                        classOut.print(", (void*)&ParseArgAndCall_"+eniName+", (void*)&" + eniName + "),\n");
                    }
                }else {
                    //classOut.print(", NULL),\n");
                    if((meth.method.access & Const.ACC_STATIC) != 0) {
                        ...
                        String jumpToInterpreter = "InvokeInterpreter" + returnType(meth.method.type.string);
                        classOut.print(", NULL, (void*)&" + jumpToInterpreter + "),\n");
                    }
            ...
            // LG DTV - BY Clamp (JNI Update) *****
            if(jniUpdate) {
                classOut.print("(CVMJavaMethodDescriptor*)&ParseArgAndCall_" +
                        meth.method.getNativeName(true)+", NULL),\n");
                printParseArgAndCallFunction(Util.convertToClassName(c.classInfo.className), meth.method.getNativeName(true), meth.method.type.string, meth.method.isStaticMember());
            }else {
            // ************************************
            ...
            if(ClassFilter.AOTC_LIST != 0 && AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.WRAPPER_TYPE) {
                if((meth.method.access & Const.ACC_AOTC) != 0) {
                    //classOut.print(", (void*)&ParseArgAndCall_"+meth.method.getNativeName(true)+"),\n");
                    if((meth.method.access & Const.ACC_STATIC) != 0) {
                        classOut.print(", (void*)&ParseArgAndCall_"+meth.method.getNativeName(true)+", NULL),\n");
                    }else {
                        classOut.print(", (void*)&ParseArgAndCall_"+meth.method.getNativeName(true)+", (void*)&" + meth.method.getNativeName(true) + "),\n");
                    }
                    ...
                    if((meth.method.access & Const.ACC_STATIC) != 0) {
                        ...
                        String jumpToInterpreter = "InvokeInterpreter" + returnType(meth.method.type.string);
                        //classOut.print(", NULL, (void*)&" + jumpToInterpreter + "),\n");
                        classOut.print(", NULL, (void*)&InvokeJNI_" + meth.method.getNativeName(true) + "),\n");
                        printInvokeAOTC(Util.convertToClassName(c.classInfo.className), meth.method.getNativeName(true), meth.method.type.string);
                    }
                }
            }else {
                classOut.print("),\n");
	...
	*/
	...
	if ( headerFileName != null ){
        ...
        if(ClassFilter.AOTC_LIST != 0 && AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.WRAPPER_TYPE) {
            o.println("#include \"romjavaAotcFunctions.h\"");
        }
    ...
    // AOTC - BY Clamp *****************************************************************
    String className = c.classInfo.className;
    //if(!isClassCNI(c.ci.className) && AOTCWriter.inList(className)) {
    if(ClassFilter.inList(className, null, null)) {
        String regEx = "\\$";
        String fname = filename+"Aotc_"+className.replace('/', '_').replaceAll(regEx,"_00024").replaceAll(";", "_00025") +".c";
        AOTCWriter.open(fname);
        listOut.println(fname);
        System.out.println("[" + aotc_num + "] Class : " + className);
        aotc_num++;
        writeMethods(c);
        AOTCWriter.close();
	...
	try {
        ...
        // AOTC - BY Clamp *****************************************************************
        String fname = filename+"AotcFunctions.h";
        AOTCWriter.writeAOTCHeader(fname, "AotcFunctions.h");
        fname = filename+"AotcFunctions.c";
        AOTCWriter.writeAOTCFunctions(fname, "AotcFunctions.c");
        listOut.println(fname);
        if(AOTCWriter.METHOD_ARGUMENT_PASSING == AOTCWriter.WRAPPER_TYPE) {
            System.out.println("fileName is " + filename);
            fname = filename + "AotcAssembly.S";
            AOTCWriter.openAssemblyWriter(fname);
    ...
    public void printSpaceStats(java.io.PrintStream o) {
     ...
     // LG DTV - BY Clamp (JNI Update) *****
    public void printParseArgAndCallFunction(String className, String methodName, String signature, boolean isStaticMember) {
        jniOut.println("#include \"generated/jni/" + className + ".h\"");
        jniOut.println("CVMInt8 ParseArgAndCall_" + methodName + "(JNIEnv* env, CVMStackVal32* arguments)");
        jniOut.println("{");
        String args = "(env, ";
        boolean isVirtual = false;
        int i = 0;
        if(isStaticMember) {
            jniOut.println("extern const CVMClassBlock " + className + "_Classblock;");
            args += className + "_Classblock.javaInstanceX";
        }else {
            i++;
            args += "&arguments[0].ref";
        }
        ...
        while((c = signature.charAt(index)) != ')') {
            ...
            switch(c) {
                case 'L': case '[':
                    args += "(arguments[" + i + "].ref.ref_DONT_ACCESS_DIRECTLY ? &arguments[" + i + "].ref : NULL)";
                    break;
                case 'F':
                    args += "arguments[" + i + "].j.f";
                    break;
                case 'D':
                    jniOut.println("CVMJavaDouble double" + i + ";");
                    JVMTo64 += ("JNIjvm2Double(arguments+"+i+",double"+i+");");
                    args += ("double"+i);
                    i++; 
                    break;
                case 'J':
                    jniOut.println("CVMJavaLong long"+i+";");
                    JVMTo64 += ("JNIjvm2Long(arguments+"+i+",long"+i+");");
                    args += ("long"+i);
                    i++; 
                    break;
                default:
        ...
        if(!JVMTo64.equals("")) {
            jniOut.println(JVMTo64);
        }
        args += ");";
        String call = (methodName + args);
        char retType = signature.charAt(signature.indexOf(')') + 1);
        switch(retType) {
            case 'L': case '[':
                jniOut.println("jobject ret = (jobject)" + call);
                jniOut.println("(&arguments[0].ref)->ref_DONT_ACCESS_DIRECTLY = ret ? ret->ref_DONT_ACCESS_DIRECTLY : NULL;");
                jniOut.println("return 1;");
                break;
            case 'D':
                jniOut.println("CVMJavaDouble ret = " + call);
                jniOut.println("JNIDouble2jvm(arguments, ret);");
                jniOut.println("return 2;");
                break;
            case 'F':
                jniOut.println("CVMJavaFloat ret = " + call);
                jniOut.println("arguments[0].j.f = ret;");
                jniOut.println("return 1;");
                break;
            case 'J':
                jniOut.println("CVMJavaLong ret = " + call);
                jniOut.println("arguments[0].j.i = ret & 0xffffffff;");
                jniOut.println("arguments[1].j.i = ((ret >> 32) & 0xffffffff);");
                jniOut.println("return 2;");
                break;
            case 'V':
                jniOut.println(call);
                jniOut.println("return 0;");
                break;
            default:
                jniOut.println("arguments[0].j.i = " + call);
                jniOut.println("return 1;");
                break;
        }
        jniOut.println("}\n");
    ...
    // LG DTV - BY Clamp (JNI Update) *****
    public void printInvokeAOTC(String className, String methodName, String signature) {
        //if(AOTCWriter.METHOD_ARGUMENT_PASSING != AOTCWriter.WRAPPER_TYPE) return;
        String inputArgs = "CVMExecEnv* ee , jobject cls_ICell";
        String invokeArgs = "CVMexecEnv2JniEnv(ee) , cls_ICell";
        String retType;

        char c;
        int i = 0, index = signature.indexOf('(') + 1;
        while((c = signature.charAt(index)) != ')') {
            inputArgs += " , ";
            invokeArgs += " , ";
            switch(c) {
                case 'L': case '[':
                    inputArgs += "jobject arg" + i;
                    invokeArgs += "(arg" + i + " ? arg" + i + " : NULL)";
                    break;
                case 'F':
                    inputArgs += "jfloat arg" + i;
                    invokeArgs += "arg" + i;
                    break;
                case 'D':
                    inputArgs += "jdouble arg" + i;
                    invokeArgs += "arg" + i;
                    break;
                case 'J':
                    inputArgs += "jlong arg" + i;
                    invokeArgs += "arg" + i;
                    break;
                default:
                    inputArgs += "jint arg" + i;
                    invokeArgs += "arg" + i;
                    break;
        ...
        jniOut.println("#include \"generated/jni/" + className + ".h\"");
        jniOut.println(retType + " InvokeJNI_" + methodName + "(" + inputArgs + ")");
        jniOut.println("{");

        if(ret == 'L') jniOut.println("jobject ret;");
        else if(ret != 'V') jniOut.println(retType + " ret;");

        jniOut.println("CVMD_gcSafeExec(ee, {");

        if(ret == 'V') {
            jniOut.println(methodName + "(" + invokeArgs + ");");
        }else {
            jniOut.println("ret = " + methodName + "(" + invokeArgs + ");");
        }

        jniOut.println("} );");
        //jniOut.println("fprintf(stderr, \" JNI INVOKE: " + methodName + "\");");
        if(ret == 'L') jniOut.println("return (ret ? ret->ref_DONT_ACCESS_DIRECTLY : NULL);");
        else if(ret != 'V') jniOut.println("return ret;");
        jniOut.println("}");
    }
    private char returnType(String signature) {
        char retType = signature.charAt(signature.indexOf(')') + 1);
        switch(retType) {
            case 'L': case '[':
                return 'L';
            case 'D': case 'F': case 'J': case 'V':
                return retType;
            default:
                return 'I';
        }
    ...
    private void writeJNIPrologue( BufferedPrintStream o ) {
        o.println("#define JNIjvm2Double(location, value) {   \\");
        o.println("    union { \\");
        o.println("        CVMJavaLong l;\\");
        o.println("        CVMJavaDouble d;    \\");
        o.println("        CVMUint32 v[2];   \\");
        o.println("    }res;   \\");
        o.println("    res.v[0] = (location)[0].j.raw;   \\");
        o.println("    res.v[1] = (location)[1].j.raw;   \\");
        o.println("    value = res.d;  \\");
        o.println("}");
        o.println("#define JNIDouble2jvm(location, value) {    \\");
        o.println("    union { \\");
        o.println("        CVMJavaLong l;\\");
        o.println("        CVMJavaDouble d;    \\");
        o.println("        CVMUint32 v[2]; \\");
        o.println("    }res;   \\");
        o.println("    res.d = value;  \\");
        o.println("    (location)[0].j.raw = res.v[0]; \\");
        o.println("    (location)[1].j.raw = res.v[1]; \\");
        o.println("}");
        o.println("#define JNIjvm2Long(location, value) {   \\");
        o.println("    union { \\");
        o.println("        CVMJavaLong d;    \\");
        o.println("        CVMUint32 v[2];   \\");
        o.println("    }res;   \\");
        o.println("    res.v[0] = (location)[0].j.raw;   \\");
        o.println("    res.v[1] = (location)[1].j.raw;   \\");
        o.println("    value = res.d;  \\");
        o.println("}");
        o.println("#define JNILong2jvm(location, value) {    \\");
        o.println("    union { \\");
        o.println("        CVMJavaLong d;    \\");
        o.println("        CVMUint32 v[2]; \\");
        o.println("    }res;   \\");
        ...
        o.println("}");
        o.println("");
        o.println("#include \"javavm/export/jvm.h\"");
        o.println("#include \"javavm/include/defs.h\"");
        o.println("#include \"javavm/include/objects.h\"");
        o.println("#include \"javavm/include/classes.h\"");
        o.println("#include \"javavm/include/interpreter.h\"");
        o.println("#include \"javavm/include/jni_impl.h\"");
        o.println("#include \"javavm/include/typeid.h\"");
        o.println("#include \"javavm/include/typeid_impl.h\"");
        o.println("#include \"javavm/include/string_impl.h\"");
        o.println("#include \"javavm/include/preloader_impl.h\"");
        o.println("#include \"javavm/include/porting/endianness.h\"");
        o.println("#include \"javavm/include/directmem.h\"");
    }
    // ************************************
    // LG DTV - BY Clamp (CVM AOT) *****
    private void writeSHA1( BufferedPrintStream o ) {
        ...
        while(value.length() < 40) {
            char ch = (char)(ran.nextInt(gap) + start);
            if(Character.isLetterOrDigit(ch)) {
                value = value + ch;
            }
        }
        o.println("static const char *cvmsha1sum = \"" + value + "\";");
        o.println("const char* CVMgetSha1Hash() {");
        o.println("    return cvmsha1sum;");
        o.println("}");
    }
