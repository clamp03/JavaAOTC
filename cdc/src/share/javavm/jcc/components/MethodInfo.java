...
class MethodInfo extends ClassMemberInfo implements Const, Cloneable
{
    ...
    for(int j = 0; j < nmethods; j ++ ) {
        ...
        if((m.access&Const.ACC_NATIVE) != 0 && (m.access&Const.ACC_AOTC) == 0) {
            if(m.name.equals(this.name)) count++;
        }
    }
    if(count == 1 && (this.access&Const.ACC_AOTC) == 0)
    ...
    // AOTC - BY Clamp ***************************************************************
    if(ClassFilter.inList(p.className, m.name.string, m.type.string)) {
        if((m.access & Const.ACC_NATIVE) == 0) {
            m.access |= Const.ACC_NATIVE;
            m.access |= Const.ACC_AOTC;
        }
    }
    // *******************************************************************************
    ...
    public void relocateAndPackCode (ConstantPool cp,
        ...
        if (code == null)
            return;
        // AOTC - BY Clamp *******************************************
        Hashtable newTypeInfo = new Hashtable();
        // ***********************************************************
	...
	ConstantObject[] co = cp.getConstants();
	    ...
	    int inPos = pc;
        // AOTC - BY Clamp *******************************************
        Object oo = (Object)typeInfo.get(Integer.toString(inPos));
        if(oo!=null) {
            newTypeInfo.put(Integer.toString(outPos), oo);
        }
        // ***********************************************************
