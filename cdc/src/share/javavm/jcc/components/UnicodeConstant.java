...
class UnicodeConstant extends ConstantObject
{
    ...
    // Clamp for mpegaudio ********************
    public boolean replaceClassMemberString() {
        int i = 0;
        ...
        while( i < string.length()) {
            ...
            if(!((charNum >= (int)'0' && charNum <= (int)'9') ||
                        ...
                        (charNum == '$') ||
                        false)) {
                String cString= (charNum) + "";
                System.out.println("Int C: \"" + charNum + "\"   C: \"" + c + "\"  String: \"" + string + "\"");
                StringBuffer sb = new StringBuffer(string.replace(c, '_'));
                sb.insert(i+1, cString);
