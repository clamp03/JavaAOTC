...
// AOTC - BY Clamp ************************
import aotc.translation.AOTCInvoke;
import aotc.*;
// ****************************************
...
*/
		...
		ClassTable.setClassLoader(l);
            ...
            // AOTC - BY Clamp **************************
            }else if (clist[i].equals("-app")) {
                CVMMethodInfo.inlining = CVMMethodInfo.NO_INLINE_FLAG;
                ClassFilter.AOTC_LIST = 3;
            }else if (clist[i].equals("-all")) {
                CVMMethodInfo.inlining = CVMMethodInfo.NO_INLINE_FLAG;
                ClassFilter.AOTC_LIST = 1;
            }else if (clist[i].equals("-none")) {
                ClassFilter.AOTC_LIST = 0;
            }else if (clist[i].equals("-call_count")) {
                CVMMethodInfo.inlining = CVMMethodInfo.NO_INLINE_FLAG;
                ClassFilter.AOTC_LIST = 4;
            }else if (clist[i].equals("-run_time")) {
                CVMMethodInfo.inlining = CVMMethodInfo.NO_INLINE_FLAG;
                ClassFilter.AOTC_LIST = 5;
            }else if (clist[i].equals("-loop_count")) {
                CVMMethodInfo.inlining = CVMMethodInfo.NO_INLINE_FLAG;
                ClassFilter.AOTC_LIST = 6;
            }else if (clist[i].equals("-test")) {
                CVMMethodInfo.inlining = CVMMethodInfo.NO_INLINE_FLAG;
                ClassFilter.AOTC_LIST = 2;
            }else if (clist[i].equals("-jni")) {
                AOTCWriter.METHOD_ARGUMENT_PASSING = AOTCWriter.JNI_TYPE;
            }else if (clist[i].equals("-cni")) {
                AOTCWriter.METHOD_ARGUMENT_PASSING = AOTCWriter.CNI_TYPE;
            }else if (clist[i].equals("-mix")) {
                AOTCWriter.METHOD_ARGUMENT_PASSING = AOTCWriter.MIX_TYPE;
            }else if (clist[i].equals("-wrapper")) {
                AOTCWriter.METHOD_ARGUMENT_PASSING = AOTCWriter.WRAPPER_TYPE;
            }else if (clist[i].equals("-check")) {
                AOTCWriter.EXCEPTION_IMPLEMENTATION = AOTCWriter.CALL_CHECK_HANDLING;
            }else if (clist[i].equals("-setjmp_prolog")) {
                AOTCWriter.EXCEPTION_IMPLEMENTATION = AOTCWriter.SETJMP_PROLOG_HANDLING;
            }else if (clist[i].equals("-setjmp_try")) {
                AOTCWriter.EXCEPTION_IMPLEMENTATION = AOTCWriter.SETJMP_TRY_HANDLING;
            // ******************************************
        // LG DTV - BY Clamp (JNI Update & AOT) *****
        } else if (clist[i].equals("-jniupdate")) {
            jniUpdate = true;
        } else if (clist[i].equals("-cvm_aot")) {
            cvmAot = true;
        // ************************************
	    } else {
    ...
    w.init(classDebug, qlossless, nativeTypes, verbosity>0,
            maxSegmentSize, true, jniUpdate, cvmAot/*jniUpdate&cvmAot arg added*/);
    // ***********************************************
	...
	for ( int i = 0; i < nclasses; i++ ){
		...
		try {
            ...
            didWorkForNatives = hd.dumpHeader( ci, o, jniUpdate/* jni Update arg added */ );
            // ************************************
            // LG DTV - BY Clamp (JNI Update, Commented) *****
