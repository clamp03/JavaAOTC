...
// SETJMP - BY Clamp *****
#ifdef SERVER_AOTC_SETJMP_EXCEPTION_HANDLING
struct AOTCHandlerList {                                    
    jmp_buf handle_block;                        
    struct AOTCHandlerList *next;       
};                                                              
typedef struct AOTCHandlerList AOTCHandlerList;

struct AOTCLockList {                                               
    jobject lock_object;            
    struct AOTCLockList *next;                      
};                           
typedef struct AOTCLockList AOTCLockList;        
#endif
// ***********************
/****************************************************************************
 * CVMExecEnv
...
#define CVM_PINNED_OBJMON_COUNT 16
    ...
    // SETJMP - BY Clamp *****
#ifdef SERVER_AOTC_SETJMP_EXCEPTION_HANDLING
    AOTCHandlerList* handlerList;
    AOTCLockList* lockList;
#endif
    // ***********************
#ifdef SERVER_AOTC // 110901 - Clamp
    CVMMethodBlock* callee;
#endif
...
#define CVMgcUnsafeThrowLocalExceptionNoDumpStack(ee, obj) {    \
    ...
    global_gc_and_exception_count++; /* AOTC - BY Clamp */ \
...
#endif /* CVM_CLASSLOADING */
#if defined(SERVER_AOTC) && defined(SERVER_AOTC_WRAPPER)
typedef int AOTNativeMethod(CVMExecEnv* ee,
        CVMStackVal32* arguments);
#endif
#if defined(SERVER_AOTC) && defined(SERVER_AOTC_MIX)
// Call Arg - BY Clamp *****
typedef void MixedMethod(CVMExecEnv* ee,
        CVMStackVal32* arguments,
        int* ret, int a, int b, int c, int d, int e, int f, int g, int h, int i, int j);
// ****************************
#endif
