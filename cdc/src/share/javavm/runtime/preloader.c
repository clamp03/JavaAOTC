...
setJitInvoker(CVMMethodBlock* mb)
{
	...
	if (CVMmbInvokerIdx(mb) == CVM_INVOKE_CNI_METHOD) {
        ...
        CVMmbJitInvoker(mb) = (void*)CVMCCMinvokeAOTCMethod;
...
#define CVM_DO_MB_UNPACK_FROM_COMPRESSED(fieldName) \
    ...
    CVM_DO_MB_UNPACK_FROM_COMPRESSED(aotInvoker);
    CVM_DO_MB_UNPACK_FROM_COMPRESSED(invokerAOT);
#endif
...
#ifdef SERVER_AOTC
	    if (CVMmbInvokerIdx(mb) < CVM_INVOKE_AOTC_METHOD) {
#else
	    if (CVMmbInvokerIdx(mb) < CVM_INVOKE_CNI_METHOD) {
#endif
