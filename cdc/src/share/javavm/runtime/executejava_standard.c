   ...
   callmethod:
	ASMLABEL(label_callmethod);
	{
...
#ifdef AOTC_PROFILE_CALL_COUNT
        {
            ...
            char cn[255];
            if(writed == 0) {
                if (CVMmbInvokerIdx(frame->mb) < CVM_INVOKE_CNI_METHOD && CVMframeIsJava(frame))
                {
                    increaseCallCounter(frame->mb, (CVMUint32)(pc - CVMjmdCode(CVMmbJmd(frame->mb))), mb);
                    /*
                       if(strcmp("ProgramInfo", CVMclassname2String(CVMcbClassName(CVMmbClassBlock(mb)), cn, 255)) == 0) {
                       fprintf(stderr, "=========== START: WRITE PROFILE INFORMATION ==========\n");
                       writeProfileInfo();
                       writed = 1;
                       fprintf(stderr, "=========== FINISH: WRITE PROFILE INFORMATION ==========\n");
                       }
                     */
                }
            }
